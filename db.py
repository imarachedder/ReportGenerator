...
class Query:
    ...
    def get_dad_datas (self, road_name):
        """
         Вытаскиваем данные из бд и формируем словарь по атрибутам
        :param name: название автомобильной дороги 
        :return: возвращаем словарь с данными по диагностике
        ...
        for i, param in enumerate(self.cursor.fetchall()):
            ...
            if tuple_km:
                ...
                km = ((0, param[-2]), (0, param[-1]))
            coordinates = (*param[3::], *km)
            #icecream.ic('DAD', coordinates)
            if param[0] in res:
                if param[1] in res.get(param[0]):
                    if param[2] in res.get(param[0]).get(param[1]):
                        res.get(param[0]).get(param[1]).get(param[2]).append(coordinates)
                    else:
                        res.get(param[0]).get(param[1]).update({param[2]: [coordinates]})
                else:
                    res.get(param[0]).update({param[1]: {param[2]: [coordinates]}})
            else:
                res.update({param[0]: {param[1]: {param[2]: [coordinates]}}})
        sort_dict_binding(res)
        #print([(i.keys()) for i in res.values() if isinstance(i, dict)])
        return dict(sorted(res.items()))
...
def main ():
    ...
    print(data_dad)
    #print(data_tp.get('весь участок').get('Километровые знаки'))
   #python -m cProfile -s cumulative some-db.py запускать в терминале для оценки времени работы скрипта
